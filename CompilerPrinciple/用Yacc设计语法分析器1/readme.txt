...
该语法分析器的词法分析器由lex编写，本例并未给出lex规则源程序，只给出了flex编译后的c语言程序lex.yy.c。本词法分析器实际上可以识别课本P178页上机题所给语言的所有记号，另外还支持以(*开始，以*)结束的注释（不支持注释的嵌套）。
...
parser.y是语法分析器的文法说明文件。该文法说明文件中的终结符声明包含了课本P178页上机题所给语言的所有记号（终结符）。

testcases文件夹下的文件为语法分析器的测试例，test0.p为当前语法分析器可通过的测试例，test1.p为P178页上机题所示文法的一个测试例。你写的其他测试例也应该放在该文件夹下。
