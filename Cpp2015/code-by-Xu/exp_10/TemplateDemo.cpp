...
public:
	...
	bool operator >(Point & point) {
		return (distanceTo() > point.distanceTo());
	}
...
public:
	...
	bool operator >(const Employee & employee) const {
		return employee.getBirthDate() > birthDate;
	}
private:
	string firstName;
	string lastName;
...
int main() {
	Point points[5] = { Point(1, 2), Point(3, 4), Point(2, 3), Point(-1, 0), Point(1, -1) };
	cout << "最远的点是";
	findMax(points, 5).print();

	Date dates[5] = { Date(1969, 8, 11), Date(1970, 7, 22), Date(1974, 11, 8), Date(1968, 2, 24), Date(1981, 1, 20) };
	cout << "最晚的日期是";
	findMax(dates, 5).printFullYear();
