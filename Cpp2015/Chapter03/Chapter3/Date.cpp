...
public:
    ...
    int fullYearsTo(int LatterYear, int LatterMonth, int LatterDay);
    // 计算当前日期与参数日期之间相差多少天(考虑闰年)，如果参数日期在当前日期之前，返回负数。
    int daysTo(int LatterYear, int LatterMonth, int LatterDay);
private:
    int year;
    int month;
    int day;
...
int Date::daysTo(int LatterYear, int LatterMonth, int LatterDay){
    ...
    if(!(LatterYear%400==0||(LatterYear%4==0&&LatterYear%100!=0)))
        JudgeMonth[2]=28;
    for(iMonth=0;iMonth<LatterMonth;iMonth++)
        LastYearDay=LastYearDay+JudgeMonth[iMonth];
    LastYearDay=LastYearDay+LatterDay;

    //分当前日期和设定日期的前后情况
    if(LatterYear>=year){
        for(iYear=year;iYear<LatterYear;iYear++)
        ...
        for(iYear=LatterYear;iYear<year;iYear++)
            if(iYear%400==0||(iYear%4==0&&iYear%100!=0))
                SumDay=SumDay+366;
            else
                SumDay=SumDay+365;
...
int main(){
    ...
    cout<<birthDate.fullYearsTo(2010, 4, 15)<<", Full of "<<birthDate.daysTo(2010, 4, 15)<<" years."<<endl;
    cout<<birthDate.daysTo(2010, 4, 15)<<" , Had lived "<<birthDate.daysTo(2010, 4, 15)<<" days."<<endl;
    cout<<birthDate.daysTo(1949, 10, 1)<<" , The People's Republic of China was born "<<-birthDate.daysTo(1949, 10, 1)<<" days earlier than me."<<endl;
